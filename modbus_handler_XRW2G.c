...
#define MIN_IO_REGISTER      51
#define MAX_IO_REGISTER      53

#define MIN_STROBED_REGISTER 100
...
/* return the current value of the pin, regardless on if it is an input or output */
int16 read_io(int8 ch) {
	if ( IO_LINE_XBEE_SLEEP == ch ) 
		return input_state(XBEE_SLEEP);
	else if (IO_LINE_XBEE_RTS == ch )
		return input_state(XBEE_RTS);

	/* IO_LINE_XBEE_CTS */
	return input_state(XBEE_CTS);
}

int8 write_io(int8 ch, int1 val) {
	if ( 1==config.io_mode[ch] ) 
		return ILLEGAL_DATA_VALUE;

	if ( IO_LINE_XBEE_SLEEP == ch ) 
		output_bit(XBEE_SLEEP,val);
	else if (IO_LINE_XBEE_RTS == ch )
		output_bit(XBEE_RTS,val);
	else if ( IO_LINE_XBEE_CTS == ch ) 
		output_bit(XBEE_CTS,val);
...
int16 map_modbus(int16 addr) {
	...
	if ( addr >= MIN_STROBED_REGISTER  && addr <= MAX_STROBED_REGISTER ) {
		addr -= 100;
		return strobed_data.strobed_data[(int8) addr];
	}
	...
	switch ( addr ) {
		...
		/* I/O line support on XBEE status lines */
		case 51: return (int16) read_io(IO_LINE_XBEE_SLEEP);
		case 52: return (int16) read_io(IO_LINE_XBEE_RTS);
		case 53: return (int16) read_io(IO_LINE_XBEE_CTS);
...
int8 modbus_valid_read_registers(int16 start, int16 end) {
	...
	if ( start >= MIN_IO_REGISTER && end <= MAX_IO_REGISTER+1 )
		return 1;
	...
	if ( start >= MIN_STROBED_REGISTER && end <= MAX_STROBED_REGISTER+1 ) 
		return 1;
...
exception modbus_write_register(int16 address, int16 value) {
	...
	switch ( address ) {
		/* IO lines */
		case 51: return write_io(0, value);
		case 52: return write_io(1, value);
		case 53: return write_io(2, value);
		...
		case 1007:
			...
			if ( config.modbus_speed != value ) {
				/* flip the speed in main */
		...
		case 1009:
			/* WorldData seconds */
			config.worldData_seconds=value;
			break;
		case 1010:
			if( value != 0 && ( value < 'A' || value > 'Z' ) ) 
				return ILLEGAL_DATA_VALUE;
			config.worldData_trigger_prefix=value;
			break;
		case 1011:
			config.worldData_trigger_number=value;
			break;
		/* IO */
		case 1012:
			if ( value > 1 ) 
				return ILLEGAL_DATA_VALUE;
			config.io_enable=value;
			break;
		case 1013:
			if ( value > 1 ) 
				return ILLEGAL_DATA_VALUE;
			config.io_mode[IO_LINE_XBEE_SLEEP]=value;
			break;
		case 1014:
			if ( value > 1 ) 
				return ILLEGAL_DATA_VALUE;
			config.io_mode[IO_LINE_XBEE_RTS]=value;
			break;
		case 1015:
			if ( value > 1 ) 
				return ILLEGAL_DATA_VALUE;
			config.io_mode[IO_LINE_XBEE_CTS]=value;
			break;
		case 1016:
			if ( value > 256 ) 
				return ILLEGAL_DATA_VALUE;
			config.modbus_strobe_address=value;
			break;
		case 1017:
...
//	disable_interrupts(GLOBAL);
	for ( i=0 ; i<= 42 ; i++ ) {
		strobed_data.strobed_data[i]=map_modbus((int16) i);
	}
//	enable_interrupts(GLOBAL);
...
void modbus_process(void) {
	...
	/* check for message */
	if ( modbus_kbhit() ) {
		/* check to see if we have been strobed */
		if ( config.modbus_strobe_address == modbus_rx.address && (modbus_rx.func == FUNC_READ_HOLDING_REGISTERS || modbus_rx.func == FUNC_READ_INPUT_REGISTERS ) && config.modbus_strobe_register == make16(modbus_rx.data[0],modbus_rx.data[1]) && 4 == modbus_rx.len ) {
			/* quickly copy data registers to strobed data registers */
